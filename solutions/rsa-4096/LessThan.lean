...
def circuit (P : BigIntParams p m) [Fact (p > 2)] :
    ...
    soundness := by
      ...
      set Cn : ℕ → ℕ := fun k => if k = m - 1 then 0
        else (env.get (i₀ + m + m * (B - 1) + k)).val with hCn
      -- digit bounds
      ...
      have h_limb : ∀ k : ℕ, (hk : k < m) →
        ...
        have hcout_e : Expression.eval env
            (if h : (⟨k, hk⟩ : Fin m).val = m - 1 then 0
              else var { index := i₀ + m + m * (B - 1) + (⟨k, hk⟩ : Fin m).val })
            = if k = m - 1 then (0 : F p) else env.get (i₀ + m + m * (B - 1) + k) := by
          simp only []; split <;> simp [circuit_norm]
        ...
        have hsum_lt : (input.lhs[k]'hk).val + (env.get (i₀ + k)).val
          ...
          omega
        have hcout_val : (if k = m - 1 then (0:F p) else env.get (i₀ + m + m * (B - 1) + k)).val
            = Cn k := by
          simp only [hCn]; split <;> simp
        have hrhs_lt : (input.rhs[k]'hk).val
            + (if k = m - 1 then (0:F p) else env.get (i₀ + m + m * (B - 1) + k)).val * 2 ^ B < p := by
    ...
    completeness := by
      ...
      · -- 3. per-limb field equation
        ...
        have hcout_e : Expression.eval env.toEnvironment
            (if h : i.val = m - 1 then 0 else var { index := i₀ + m + m * (B - 1) + i.val })
            = if i.val = m - 1 then (0 : F p) else env.get (i₀ + m + m * (B - 1) + i.val) := by
          split <;> simp [circuit_norm]
        ...
        have hcast_eq : (input.lhs[i.val]'hk) + env.get (i₀ + i.val)
            ...
            = (input.rhs[i.val]'hk)
              + (if i.val = m - 1 then (0:F p) else env.get (i₀ + m + m * (B - 1) + i.val))
                * (2 ^ B : F p) := by

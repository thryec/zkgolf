...
def main (P : BigIntParams p m) [Fact (p > 2)] (input : Var (Inputs m) (F p)) :
  ...
  -- 1. witness the running (offset) carries c[0 .. 2m-3] (carry out of each
  -- index except the top one, whose signed carry is identically `0` and is
  -- folded into the top constraint).
  let carry ← witnessVector (2 * m - 1 - 1) fun env =>
    Vector.ofFn fun k : Fin (2 * m - 1 - 1) =>
  ...
  -- 2. range-check each carry to `W` bits with an implicit top bit (subcircuit call).
  have hWpos : 1 ≤ P.W := by
    ...
    have hpos : 0 < carryOffset (m := m) B * 2 := by
      ...
      positivity
    omega
  Circuit.forEach carry (fun c => RangeCheck.circuit P.W P.hW hWpos c)

  -- 3. per-index linear constraint (signed carries via the offset convention)
  --    `lhs[k] + (carry[k-1] − OFF) − rhs[k] − (carry[k] − OFF)·2^B = 0`,
  --    with effective `carry_in = 0` for `k = 0`, and the top index's outgoing
  --    (signed) carry fixed to `0` (the forced top carry is folded into the top
  --    constraint rather than asserted separately). Built purely, then asserted.
  let constraints : Vector (Expression (F p)) (2 * m - 1) :=
    Vector.mapFinRange (2 * m - 1) fun k =>
      ...
      let carryOut : Expression (F p) :=
        if h : k.val = 2 * m - 1 - 1 then 0
        else carry[k.val]'(by omega) - (carryOffset (m := m) P.B : F p)
      Pc[k.val] + carryIn - Sc[k.val] - carryOut * (2 ^ P.B : F p)
...
instance elaborated (P : BigIntParams p m) [Fact (p > 2)] :
  ...
  -- carries: (2m-1) witnesses + (2m-1) * (W-1) range-check bits
  localLength _ := (2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1)
  localLength_eq := by
    intro input offset
...
def circuit (P : BigIntParams p m) [Fact (p > 2)] : FormalAssertion (F p) (Inputs m) where
    ...
    soundness := by
      ...
      -- nat-indexed coefficient / carry functions. The top index's *effective*
      -- (offset) carry-out is `OFFn` by construction: the top constraint has no
      -- carry-out term, which encodes a signed carry-out of `0`.
      ...
      set Cn : ℕ → ℕ := fun k => if k = 2 * m - 1 - 1 then OFFn
        else (env.get (i₀ + k)).val with hCn
      have hOFF_pos : 0 < OFFn := by
        have : OFFn = (m + 1) * (2 ^ B + 2) := rfl
        rw [this]; positivity
      -- bound facts
      have hCn_lt : ∀ k, k < 2 * m - 1 → Cn k < 2 ^ W := by
        intro k hk
        by_cases hktop : k = 2 * m - 1 - 1
        · simp only [hCn, if_pos hktop]
          have := hWB
          omega
        · simp only [hCn, if_neg hktop]
          simpa using h_range ⟨k, by omega⟩ trivial
      ...
      -- key algebraic facts about OFFn = (m+1)*(2^B + 2)
      have hOFF_eq : OFFn = (m + 1) * (2 ^ B + 2) := rfl
      have h2B : (2 : ℕ) ≤ 2 ^ B := by
        calc (2 : ℕ) = 2 ^ 1 := (pow_one 2).symm
          _ ≤ 2 ^ B := Nat.pow_le_pow_right (by norm_num) hB1
      have hXeq : (2 : ℕ) ^ (2 * B) = 2 ^ B * 2 ^ B := by rw [← pow_add]; congr 1; ring
      -- OFFn·2^B = (m+1)·(2^(2B) + 2^(B+1)) ≤ (m+1)·2^(2B)·3
      have hOFFB_le : OFFn * 2 ^ B ≤ (m + 1) * 2 ^ (2 * B) * 3 := by
        rw [hOFF_eq]
        calc (m + 1) * (2 ^ B + 2) * 2 ^ B
            ≤ (m + 1) * (2 ^ B + 2 ^ B) * 2 ^ B := by
              refine Nat.mul_le_mul_right _ (Nat.mul_le_mul_left _ (by omega))
          _ = (m + 1) * 2 ^ (2 * B) * 2 := by rw [hXeq]; ring
          _ ≤ (m + 1) * 2 ^ (2 * B) * 3 := Nat.mul_le_mul_left _ (by norm_num)
      have hOFF_le3 : OFFn ≤ (m + 1) * 2 ^ (2 * B) * 3 := by
        have h1 : OFFn ≤ OFFn * 2 ^ B := Nat.le_mul_of_pos_right _ (Nat.two_pow_pos B)
        omega
      ...
      have hXW : (m + 1) * 2 ^ (2 * B) ≤ 2 ^ W * 2 ^ B := by
        have h1 : (m + 1) * 2 ^ B ≤ 2 ^ W := by
          have h2 : (m + 1) * 2 ^ B ≤ OFFn * 2 := by
            rw [hOFF_eq]
            calc (m + 1) * 2 ^ B ≤ (m + 1) * ((2 ^ B + 2) * 2) := by
                  refine Nat.mul_le_mul_left _ (by omega)
              _ = (m + 1) * (2 ^ B + 2) * 2 := by ring
          omega
        calc (m + 1) * 2 ^ (2 * B) = (m + 1) * 2 ^ B * 2 ^ B := by rw [hXeq]; ring
          _ ≤ 2 ^ W * 2 ^ B := Nat.mul_le_mul_right _ h1
      -- the top outgoing (offset) carry is `OFFn` by construction (folded into
      -- the top constraint)
      ...
      have h_idx : ∀ k, (hk : k < 2 * m - 1) →
        ...
        -- the top index's carry-out field value is the constant `OFFn`
        set cF : F p := if k = 2 * m - 1 - 1 then (OFFn : F p) else env.get (i₀ + k) with hcF
        have hcF_val : cF.val = Cn k := by
          simp only [hcF, hCn]; split
          · exact hOFFn_cast
          · rfl
        ...
        have hcout_e : Expression.eval env
            (if h : (⟨k, hk⟩ : Fin (2*m-1)).val = 2 * m - 1 - 1 then 0
              else var { index := i₀ + (⟨k, hk⟩ : Fin (2*m-1)).val } - Expression.const (OFFn : F p))
            = if k = 2 * m - 1 - 1 then 0 else env.get (i₀ + k) - (OFFn : F p) := by
          simp only []
          split <;> simp [circuit_norm, sub_eq_add_neg]
        simp only [ha_e, hb_e, hcin_e, hcout_e] at hlin
        -- bridge the goal's carry-out ite to `cF − OFF` (an atom for `ring`)
        have hbridge : (if k = 2 * m - 1 - 1 then (0:F p) else env.get (i₀ + k) - (OFFn : F p))
            = cF - (OFFn : F p) := by
          simp only [hcF]; split <;> ring
        rw [hbridge] at hlin
        -- the unified field equation
        have hfield : (input.lhs[k]'hk) + (if k = 0 then (OFFn : F p) else env.get (i₀ + (k - 1)))
            + (OFFn : F p) * (2 ^ B : F p)
            = (input.rhs[k]'hk) + cF * (2 ^ B : F p) + (OFFn : F p) := by
        ...
        have hcin_val : (if k = 0 then (OFFn : F p) else env.get (i₀ + (k - 1))).val
          ...
          · exact hOFFn_cast
          · rename_i hkne
            simp only [hCn, if_neg (show ¬ (k - 1 = 2 * m - 1 - 1) by omega)]
    ...
    completeness := by
      ...
      -- the witnessed value equals Cn (for the witnessed indices)
      have hwit_eq : ∀ k, k < 2 * m - 1 - 1 → env.get (i₀ + k) = (Cn k : F p) := by
        intro k hk
        rw [h_wit ⟨k, hk⟩]
      ...
      have hidx : ∀ k, k < 2 * m - 1 →
        ...
        · rw [if_neg (by omega : ¬ k = 0), hmulCnk]
          ...
          omega
      -- carry val facts
      have hCn_val : ∀ k, k < 2 * m - 1 - 1 → (env.get (i₀ + k)).val = Cn k := by
        intro k hk
      ...
      · -- range check
        intro i
        exact ⟨trivial, by
          change (env.get (i₀ + i.val)).val < 2 ^ W
          rw [hCn_val i.val i.isLt]
          exact hrange i.val⟩
      · -- per-index field equation
        ...
        have hcout_e : Expression.eval env.toEnvironment
            (if h : i.val = 2 * m - 1 - 1 then 0
              else var { index := i₀ + i.val } - Expression.const (OFFn : F p))
            = if i.val = 2 * m - 1 - 1 then 0 else env.get (i₀ + i.val) - (OFFn : F p) := by
          split <;> simp [circuit_norm, sub_eq_add_neg]
        ...
        have hCk : ((Cn i.val : ℕ) : F p)
            = (if i.val = 2 * m - 1 - 1 then (OFFn : F p) else env.get (i₀ + i.val)) := by
          split
          · rename_i htop
            rw [htop, show 2 * m - 1 - 1 = 2 * m - 2 from by omega, hCtop]
          · rename_i htop
            rw [hwit_eq i.val (by omega)]
        ...
        by_cases hktop : i.val = 2 * m - 1 - 1
        · -- top index: the constraint has no carry-out term; the semantic top
          -- carry equals OFF, so the `OFF·2^B` terms cancel by `linear_combination`
          rw [if_pos hktop]
          simp only [if_pos hktop] at hcast
          ...
          · simp only [if_neg (by omega : ¬ i.val = 0)] at hcast ⊢
            ...
            linear_combination hcast
        · rw [if_neg hktop]
          simp only [if_neg hktop] at hcast
          rcases Nat.eq_zero_or_pos i.val with hi0 | hi0
          · simp only [hi0, ↓reduceIte, add_zero] at hcast ⊢

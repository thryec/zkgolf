...
/-- `localLength` of a single `MulModLazy` subcircuit (mirrors `MulModLazy.elaborated`).
...
def mulModLen (B W tb : ℕ) : ℕ :=
  m + m + m * (B - 1) + ((m - 1) * (B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1)
    + ((2 * m - 1 - 1) * (W - 1) + (2 * m - 1 - 1))

/-- `localLength` of a single `SquareModLazy` subcircuit (mirrors
`SquareModLazy.elaborated`): the `a·a` product is witnessed symmetrically
(`tri m = m(m+1)/2` cells) rather than as a full `m*m` matrix. -/
def squareModLen (B W tb : ℕ) : ℕ :=
  m + m + ((m - 1) * (B - 1) + (tb + 2 - 1)) + ((m - 1) * (B - 1) + (tb - 1))
    + (2 * m - 1) + (2 * m - 1)
    + ((2 * m - 1 - 1) * (W - 1) + (2 * m - 1 - 1))
...
def modExpLoop (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
  ...
  | bit :: rest, acc => do
      ...
      modExpLoop P tb htb htbB base n rest acc'
...
def main (P : RSAParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.bigIntParams.B)
    [Fact (p > 2)]
  ...
  | _ :: tail => modExpLoop P.bigIntParams tb htb htbB base n tail base

/-- `localLength` of the unrolled loop over a bit list: one `MulMod` per bit
(squaring) plus one extra `MulMod` per set bit (multiply). -/
lemma modExpLoop_localLength (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
    (base n : Var (BigInt m) (F p)) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      (modExpLoop P tb htb htbB base n bs acc).localLength offset
        = bs.length * squareModLen (m := m) P.B P.W tb
            + bs.count true * mulModLen (m := m) P.B P.W tb := by
  intro bs
  induction bs with
  ...
  | cons bit rest ih =>
    ...
    have hSL : ∀ (x : Var (SquareModLazy.Inputs m) (F p)),
        (SquareModLazy.circuit P tb htb htbB).localLength x
          = squareModLen (m := m) P.B P.W tb := fun _ => rfl
    have hML : ∀ (x : Var (MulMod.Inputs m) (F p)),
        (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))).localLength x
          = mulModLen (m := m) P.B P.W tb := fun _ => rfl
    cases bit
    · simp only [modExpLoop, circuit_norm, ih, List.length_cons, List.count_cons,
      ...
      generalize squareModLen (m := m) P.B P.W tb = S
      generalize mulModLen (m := m) P.B P.W tb = L
      ring
    · simp only [modExpLoop, circuit_norm, ih, List.length_cons, List.count_cons,
...
lemma modExpLoop_subcircuitsConsistent (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p)
    ...
    (base n : Var (BigInt m) (F p)) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      ((modExpLoop P tb htb htbB base n bs acc).operations offset).SubcircuitsConsistent offset := by
  intro bs
  induction bs with
  | nil => intro acc offset; simp [modExpLoop, circuit_norm]
  | cons bit rest ih =>
    ...
    have key : ∀ acc' off, Operations.forAll off { subcircuit := fun off {n} _ => n = off }
        ((modExpLoop P tb htb htbB base n rest acc').operations off) := by
...
lemma modExpLoop_channelsLawful (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
    (base n : Var (BigInt m) (F p)) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      ((modExpLoop P tb htb htbB base n bs acc).operations offset).ChannelsLawful [] [] := by
  -- The single-`MulModLazy`-subcircuit step is channel-lawful (R1CS, no channels).
  have hsub : ∀ (x : Var (MulMod.Inputs m) (F p)) (off : ℕ),
      (((subcircuit (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))) x)).operations off).ChannelsLawful [] [] := by
    intro x off
    simp only [circuit_norm, MulModLazy.circuit, MulModLazy.elaborated]
  have hsubSq : ∀ (x : Var (SquareModLazy.Inputs m) (F p)) (off : ℕ),
      (((subcircuit (SquareModLazy.circuit P tb htb htbB) x)).operations off).ChannelsLawful [] [] := by
    intro x off
    simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated]
  intro bs
  induction bs with
  | nil =>
    intro acc offset
  ...
  | cons bit rest ih =>
    ...
    · show ((do
          let sq ← subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }
          modExpLoop P tb htb htbB base n rest sq).operations offset).ChannelsLawful [] []
    ...
    · show ((do
          let sq ← subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }
          let acc' ← subcircuit (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))) { a := sq, b := base, modulus := n }
          modExpLoop P tb htb htbB base n rest acc').operations offset).ChannelsLawful [] []
...
lemma modExpLoop_soundness (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
    ...
    (hn_ge : 2 ^ ((m - 1) * P.B + tb) ≤ 4 * (ev env n).value P.B) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset E : ℕ),
      ...
      Operations.forAllNoOffset
        ...
        ((modExpLoop P tb htb htbB base n bs acc).operations offset) →
      (ev env (modExpLoop P tb htb htbB base n bs acc offset).1).Normalized P.B ∧
      (ev env (modExpLoop P tb htb htbB base n bs acc offset).1).value P.B < 2 ^ ((m - 1) * P.B + tb) ∧
      (ev env (modExpLoop P tb htb htbB base n bs acc offset).1).value P.B % (ev env n).value P.B
  ...
  | cons bit rest ih =>
    ...
    set sqVar : Var (BigInt m) (F p) :=
      (SquareModLazy.circuit P tb htb htbB).output { a := acc, modulus := n } offset with hsqVar
    -- SquareModLazy assumptions for the square
    have hsq_as : SquareModLazy.Assumptions P.B tb ({ a := ev env acc, modulus := ev env n } :
        SquareModLazy.Inputs m (F p)) := by
      exact ⟨hacc_norm, hn_norm, hacc_lt, hn_ltT, hn_pos, hn_ge⟩
    rw [show modExpLoop P tb htb htbB base n (bit :: rest) acc
        = (do
            let sq ← subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }
            let acc' ← if bit then subcircuit (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))) { a := sq, b := base, modulus := n }
                       else pure sq
            modExpLoop P tb htb htbB base n rest acc') from rfl] at h_holds ⊢
    ...
    simp only [circuit_norm, SquareModLazy.circuit] at h_sq_holds
    ...
    have hsq_ltT : (ev env sq).value P.B < 2 ^ ((m - 1) * P.B + tb) := BigInt.value_lt_tight (Nat.le_of_succ_le (Nat.le_of_succ_le htbB)) hsq_tight
    ...
    have hsqVar_eq : sqVar = sq := by
      simp only [hsqVar, hsq_def, circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated]
    cases bit
    · -- clear bit: acc' = sq, exponent step E ↦ 2E
      simp only [Bool.false_eq_true, if_false] at h_rest_holds ⊢
      simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated] at h_rest_holds ⊢
    ...
    · -- set bit: an extra multiply, exponent step E ↦ 2E+1
      ...
      set mulVar : Var (BigInt m) (F p) :=
        (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))).output { a := sqVar, b := base, modulus := n }
          (offset + (SquareModLazy.circuit P tb htb htbB).localLength { a := acc, modulus := n }) with hmulVar
      ...
      have hmul_as : MulModLazy.Assumptions P.B tb ({ a := ev env sq, b := ev env base, modulus := ev env n } :
          MulMod.Inputs m (F p)) :=
        ⟨hsq_norm, hbase_norm, hn_norm, hsq_ltT, hbase_ltT, hn_ltT, hn_pos, hn_big⟩
      simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated, MulModLazy.circuit] at h_mul_holds
      obtain ⟨hmul_tight, hmul_val⟩ := h_mul_holds hmul_as
      set mul : Var (BigInt m) (F p) :=
        Vector.mapRange m fun i ↦
          var { index := offset + (m + m + ((m - 1) * (P.B - 1) + (tb + 2 - 1)) + ((m - 1) * (P.B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1) + ((2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1))) + m + i }
        with hmul_def
      have hmul_norm : (ev env mul).Normalized P.B := hmul_tight.1
      have hmul_ltT : (ev env mul).value P.B < 2 ^ ((m - 1) * P.B + tb) := BigInt.value_lt_tight (Nat.le_of_succ_le (Nat.le_of_succ_le htbB)) hmul_tight
      ...
      simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated, MulModLazy.circuit,
        MulModLazy.elaborated] at h_rest2_holds ⊢
...
lemma modExpLoop_completeness (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
    ...
    (hn_ge : 2 ^ ((m - 1) * P.B + tb) ≤ 4 * (ev env.toEnvironment n).value P.B) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      ...
      Operations.forAllNoOffset
        ...
        ((modExpLoop P tb htb htbB base n bs acc).operations offset) ∧
      (ev env.toEnvironment (modExpLoop P tb htb htbB base n bs acc offset).1).Normalized P.B ∧
      (ev env.toEnvironment (modExpLoop P tb htb htbB base n bs acc offset).1).value P.B
        < 2 ^ ((m - 1) * P.B + tb) := by
  ...
  | cons bit rest ih =>
    ...
    set sqVar : Var (BigInt m) (F p) :=
      (SquareModLazy.circuit P tb htb htbB).output { a := acc, modulus := n } offset with hsqVar
    have hsq_as : SquareModLazy.Assumptions P.B tb ({ a := ev env.toEnvironment acc, modulus := ev env.toEnvironment n } :
    ...
    rw [show modExpLoop P tb htb htbB base n (bit :: rest) acc
        = (do
            ...
            modExpLoop P tb htb htbB base n rest acc') from rfl] at h_env ⊢
    ...
    simp only [circuit_norm, SquareModLazy.circuit] at h_sq_env
    ...
    have hsq_ltT : (ev env.toEnvironment sq).value P.B < 2 ^ ((m - 1) * P.B + tb) :=
      BigInt.value_lt_tight (Nat.le_of_succ_le (Nat.le_of_succ_le htbB)) hsq_tight
    have hsqVar_eq : sqVar = sq := by
      simp only [hsqVar, hsq_def, circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated]
    ...
    have hsq_pa : Operations.forAllNoOffset
        { assert := fun e ↦ Expression.eval env.toEnvironment e = 0, lookup := fun l ↦ l.Completeness env.toEnvironment,
          interact := fun i ↦ i.Guarantees env.toEnvironment, subcircuit := fun {_m} s ↦ s.ProverAssumptions env }
        ((subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }).operations offset) := by
      simp only [circuit_norm, MulModLazy.circuit]
      exact hsq_as
    simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated] at hsq_pa
    cases bit
    · -- clear bit: acc' = sq
      simp only [Bool.false_eq_true, if_false] at h_rest_env ⊢
      simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated] at h_rest_env ⊢
    ...
    · -- set bit: extra multiply
      ...
      simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated, MulModLazy.circuit,
        MulModLazy.elaborated] at h_rest_env ⊢
      obtain ⟨h_mul_spec, h_rest2_env⟩ := h_rest_env
      have hmul_as : MulModLazy.Assumptions P.B tb ({ a := ev env.toEnvironment sq, b := ev env.toEnvironment base, modulus := ev env.toEnvironment n } :
          MulMod.Inputs m (F p)) :=
        ⟨hsq_norm, hbase_norm, hn_norm, hsq_ltT, hbase_ltT, hn_ltT, hn_pos, hn_big⟩
      ...
      have hmul_ltT : (ev env.toEnvironment mul).value P.B < 2 ^ ((m - 1) * P.B + tb) :=
        BigInt.value_lt_tight (Nat.le_of_succ_le (Nat.le_of_succ_le htbB)) hmul_tight
      rw [show (m + m + ((m - 1) * (P.B - 1) + (tb + 2 - 1)) + ((m - 1) * (P.B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1) + ((2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1))) + offset +
          (m + m + m * (P.B - 1) + ((m - 1) * (P.B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1) + ((2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1)))
        = offset + (m + m + ((m - 1) * (P.B - 1) + (tb + 2 - 1)) + ((m - 1) * (P.B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1) + ((2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1))) +
          (m + m + m * (P.B - 1) + ((m - 1) * (P.B - 1) + (tb - 1)) + (2 * m - 1) + (2 * m - 1) + ((2 * m - 1 - 1) * (P.W - 1) + (2 * m - 1 - 1))) by ring] at h_rest2_env
...
lemma modExpLoop_requirements (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.B)
    [Fact (p > 2)]
    (env : Environment (F p)) (base n : Var (BigInt m) (F p)) :
    ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      Operations.forAllNoOffset
        ...
        ((modExpLoop P tb htb htbB base n bs acc).operations offset) := by
  have hsub : ∀ (x : Var (MulMod.Inputs m) (F p)) (off : ℕ),
      Operations.forAllNoOffset
        { interact := fun i ↦ i.Requirements env,
          subcircuit := fun {_m} s ↦ s.channelsWithRequirements = [] ∨ s.Assumptions env }
        ((subcircuit (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))) x).operations off) := by
    intro x off
    simp only [circuit_norm, MulModLazy.circuit, MulModLazy.elaborated]
  have hsubSq : ∀ (x : Var (SquareModLazy.Inputs m) (F p)) (off : ℕ),
      Operations.forAllNoOffset
        { interact := fun i ↦ i.Requirements env,
          subcircuit := fun {_m} s ↦ s.channelsWithRequirements = [] ∨ s.Assumptions env }
        ((subcircuit (SquareModLazy.circuit P tb htb htbB) x).operations off) := by
    intro x off
    simp only [circuit_norm, SquareModLazy.circuit, SquareModLazy.elaborated]
  intro bs
  induction bs with
  ...
  | cons bit rest ih =>
    ...
    · show Operations.forAllNoOffset _ ((do
          let sq ← subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }
          modExpLoop P tb htb htbB base n rest sq).operations offset)
      rw [Circuit.bind_operations_eq, Operations.forAllNoOffset_append]
      exact ⟨hsubSq { a := acc, modulus := n } offset, ih _ _⟩
    · show Operations.forAllNoOffset _ ((do
          let sq ← subcircuit (SquareModLazy.circuit P tb htb htbB) { a := acc, modulus := n }
          let acc' ← subcircuit (MulModLazy.circuit P tb htb (Nat.le_of_succ_le (Nat.le_of_succ_le htbB))) { a := sq, b := base, modulus := n }
          modExpLoop P tb htb htbB base n rest acc').operations offset)
      rw [Circuit.bind_operations_eq, Circuit.bind_operations_eq,
...
def modExpLen (e B W tb : ℕ) : ℕ := match eBits e with
  | [] => 0
  | _ :: tail => tail.length * squareModLen (m := m) B W tb
      + tail.count true * mulModLen (m := m) B W tb

instance elaborated (P : RSAParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.bigIntParams.B)
    [Fact (p > 2)] :
    ElaboratedCircuit (F p) (Inputs m) (BigInt m) (main P tb htb htbB) where
  localLength _ := modExpLen P.e P.bigIntParams.B P.bigIntParams.W tb
  localLength_eq := by
    intro input offset
    ...
    | cons headBit tail =>
      exact modExpLoop_localLength P.bigIntParams tb htb htbB _ _ tail _ offset
  subcircuitsConsistent := by
    ...
    | nil => simp [circuit_norm]
    | cons headBit tail =>
      exact modExpLoop_subcircuitsConsistent P.bigIntParams tb htb htbB _ _ tail _ offset
  channelsLawful := by
    intro input offset
    simp only [main]
    cases h : eBits P.e with
    | nil =>
      ...
      exact Operations.channelsLawful_nil
    | cons headBit tail =>
      exact modExpLoop_channelsLawful P.bigIntParams tb htb htbB _ _ tail _ offset
...
def Assumptions (_e B tb : ℕ)
  ...
  base.Normalized B ∧ n.Normalized B ∧
    ...
    0 < n.value B ∧ 2 ^ ((m - 1) * B + tb) ≤ 4 * n.value B
...
def circuit (P : RSAParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb + 2 ≤ P.bigIntParams.B)
    [Fact (p > 2)] :
    ...
    soundness := by
      ...
      | cons headBit tail =>
        ...
        have hbase_val : (ev env input_var.base).value P.bigIntParams.B
          ...
          rw [pow_one]
        refine ⟨?_, modExpLoop_requirements P.bigIntParams tb htb htbB env _ _ tail _ _⟩
        obtain ⟨hout_norm, hout_ltT, hout_val⟩ :=
          modExpLoop_soundness P.bigIntParams tb htb htbB env input_var.base input_var.modulus
    ...
    completeness := by
      ...
      | cons headBit tail =>
        ...
        rw [h] at h_env
        simp only at h_env ⊢
        exact (modExpLoop_completeness P.bigIntParams tb htb htbB env input_var.base input_var.modulus

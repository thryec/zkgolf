...
def evalValue (B : ℕ) (env : ProverEnvironment (F p))
...
def main (P : BigIntParams p m) [Fact (p > 2)]
  ...
  -- 3. Pc = a·b ; S = q·n + r via interpolation multiplication (affine coeffs)
...
instance elaborated (P : BigIntParams p m) [Fact (p > 2)] :
  ...
  -- q (m) + r (m) + normalize q (m*(B-1)) + normalize r (m*(B-1))
  --   + interpolatedMul a b (2m-1) + interpolatedMul q n (2m-1)
  --   + eqViaCarries ((2m-1)*(W-1) + (2m-1)) + lessThan (m + m*(B-1) + m)
  localLength _ :=
  ...
  localLength_eq := by
    ...
    simp only [main, interpolatedMul, circuit_norm, Normalize.circuit, Normalize.elaborated,
      ...
      LessThan.circuit, LessThan.elaborated, LessThan.main, RangeCheck.circuit,

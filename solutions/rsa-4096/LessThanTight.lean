...
def main (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb ≤ P.B) [Fact (p > 2)] (input : Var (Inputs m) (F p)) :
    Circuit (F p) Unit := do
  let a := input.lhs
  let b := input.rhs
  ...
  NormalizeTight.circuit P tb htb htbB d

  -- 3. witness one carry (borrow) bit per limb, except the top limb (whose
  -- outgoing carry is identically `0` and is folded into its constraint). The
  -- carry *out* of limb `k` for the base-`2^B` addition `a + d + 1` is the
  -- running carry `⌊P_k / 2^(B·(k+1))⌋` where `P_k = 1 + Σ_{j≤k} (a[j]+d[j])·2^(B·j)`
  ...
  -- boolean-constrain each carry bit except the top one: the top limb's
  -- constraint fixes its outgoing carry to `0` (see step 4), and only carries
  -- `0 .. m-2` feed into the next limb's recurrence.
  let boolCs : Vector (Expression (F p)) (m - 1) := Vector.mapFinRange (m - 1) fun k =>
    (carry[k.val]'k.isLt) * ((carry[k.val]'k.isLt) - 1)
  Circuit.forEach boolCs assertZero

  -- 4. per-limb recurrence `a[k] + d[k] + carry_in + [k=0] = b[k] + carry_out·2^B`
  -- where `carry_in = carry[k-1]` (0 for `k = 0`), and the `+1` is folded into
  -- limb 0's constant term. The top limb's outgoing carry is fixed to `0` (the
  -- forced-zero top carry is folded into the top constraint rather than asserted
  -- separately). We build the per-limb constraint expressions purely, then
  -- assert each is zero.
  let constraints : Vector (Expression (F p)) m := Vector.mapFinRange m fun k =>
    ...
    let carryOut : Expression (F p) :=
      if h : k.val = m - 1 then 0 else carry[k.val]'(by omega)
    a[k.val] + d[k.val] + carryIn + one - b[k.val] - carryOut * (2 ^ P.B : F p)
  Circuit.forEach constraints assertZero

instance elaborated (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb ≤ P.B) [Fact (p > 2)] :
    ElaboratedCircuit (F p) (Inputs m) unit (main P tb htb htbB) where
  -- d : m witnesses; NormalizeTight : (m-1)(B-1)+(tb-1); carry bits : m
  localLength _ := m + ((m - 1) * (P.B - 1) + (tb - 1)) + (m - 1)
  localLength_eq := by
    intro input offset
    simp only [main, circuit_norm, NormalizeTight.circuit, NormalizeTight.elaborated, NormalizeTight.main,
      RangeCheck.circuit]
    simp +arith [circuit_norm]
  subcircuitsConsistent := by
    intro input offset
    simp +arith only [main, circuit_norm, NormalizeTight.circuit, NormalizeTight.elaborated, NormalizeTight.main,
      RangeCheck.circuit]
  channelsLawful := by
    intro offset
    simp only [main, circuit_norm, NormalizeTight.circuit, NormalizeTight.elaborated, NormalizeTight.main,
...
def circuit (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p) (htbB : tb ≤ P.B) [Fact (p > 2)] :
    ...
    soundness := by
      ...
      circuit_proof_start
      simp only [circuit_norm, NormalizeTight.circuit, NormalizeTight.elaborated, NormalizeTight.main,
        RangeCheck.circuit] at h_holds ⊢
      obtain ⟨h_dnorm, h_cbool, h_lin⟩ := h_holds
      obtain ⟨ha_norm, hb_norm, hb_tight⟩ := h_assumptions
      ...
      set Cn : ℕ → ℕ := fun k => if k = m - 1 then 0
        else (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val with hCn
      -- digit bounds
      ...
      have hDn_lt : ∀ k, k < m → Dn k < 2 ^ B := by
        intro k hk
        have hspec := (h_dnorm trivial).1 ⟨k, hk⟩
      ...
      -- the top outgoing carry is `0` by construction (folded into the top constraint)
      have htop0 : Cn (m - 1) = 0 := by simp [hCn]
      -- carry bits: booleanity for k < m − 1, the folded top carry otherwise
      have hCn_le : ∀ k, k < m → Cn k ≤ 1 := by
        intro k hk
        rcases Nat.lt_or_ge k (m - 1) with hk1 | hk1
        · have hb := h_cbool ⟨k, hk1⟩
          have : IsBool (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)) := by
            rw [IsBool.iff_mul_sub_one]; rw [show env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k) - 1
              = env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k) + -1 by ring]; exact hb
          have := IsBool.val_lt_two this
          simp only [hCn, if_neg (by omega : ¬ k = m - 1)]; omega
      ...
      have h_limb : ∀ k : ℕ, (hk : k < m) →
        ...
        have hcin_e : Expression.eval env
            (if h : (⟨k, hk⟩ : Fin m).val = 0 then 0
              else var { index := i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + ((⟨k, hk⟩ : Fin m).val - 1) })
            = if k = 0 then 0 else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1)) := by
          simp only []
        ...
        have hcout_e : Expression.eval env
            (if h : (⟨k, hk⟩ : Fin m).val = m - 1 then 0
              else var { index := i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (⟨k, hk⟩ : Fin m).val })
            = if k = m - 1 then (0 : F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k) := by
          simp only []; split <;> simp [circuit_norm]
        simp only [ha_e, hb_e, hcin_e, hone_e, hcout_e] at hlin
        -- bounds for the lift lemma
        have h3 : (if k = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1))).val ≤ 1 := by
          split
          · simp [ZMod.val_zero]
          · rename_i h
            have hkm : k - 1 < m := by omega
            have := hCn_le (k - 1) hkm
            simp only [hCn, if_neg (show ¬ (k - 1 = m - 1) by omega)] at this; exact this
        ...
        have hsum_lt : (input.lhs[k]'hk).val + (env.get (i₀ + k)).val
            + (if k = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1))).val
            + (if k = 0 then (1 : F p) else 0).val < p := by
          have h1 := hAn_lt k hk; have h2 := hDn_lt k hk
          simp only [hAn, hDn, dif_pos hk] at h1 h2
          omega
        have hcout_val : (if k = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val
            = Cn k := by
          simp only [hCn]; split <;> simp
        have hrhs_lt : (input.rhs[k]'hk).val
            + (if k = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val * 2 ^ B < p := by
        ...
        have hlin' : (input.lhs[k]'hk) + env.get (i₀ + k)
            + (if k = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1)))
            + (if k = 0 then (1 : F p) else 0) - (input.rhs[k]'hk)
            - (if k = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)) * (2 ^ B : F p)
            = 0 := by
          rw [sub_eq_add_neg, sub_eq_add_neg]; exact hlin
        have hlift := per_limb_lift (B := B) (input.lhs[k]'hk) (env.get (i₀ + k))
          (if k = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1)))
          (if k = 0 then (1 : F p) else 0) (input.rhs[k]'hk)
          (if k = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k))
          hB hsum_lt hrhs_lt hlin'
        -- rewrite the `.val`s of the carry/one terms
        have hcin_val : (if k = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (k - 1))).val
            = if k = 0 then 0 else Cn (k - 1) := by
          split
          · simp
          · rename_i hk0
            simp only [hCn, if_neg (show ¬ (k - 1 = m - 1) by omega)]
    ...
    completeness := by
      ...
      have hdtot_tight : dtot < 2 ^ ((m - 1) * B + tb) := by
        rw [hdtot]
        omega
      ...
      have hCn_eq : ∀ k : ℕ, k < m - 1 →
          (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val = P k / 2 ^ (B * (k + 1)) := by
        intro k hk
      ...
      have hCn_bit : ∀ k : ℕ, k < m - 1 → IsBool (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)) := by
        intro k hk
        have hle : (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val ≤ 1 := by
        ...
        · right
          have : env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k) = ((1 : ℕ) : F p) := by
            apply FieldUtils.ext
            rw [h1]
            simp [ZMod.val_one]
          simpa using this
      refine ⟨?_, ?_, ?_⟩
      · -- 1. d is tightly normalized (NormalizeTight subcircuit obligation)
        refine ⟨trivial, ?_⟩
        refine ⟨?_, ?_⟩
        · intro i
          have : (Vector.map (Expression.eval env.toEnvironment)
          ...
          exact this
        · have hmpos : 0 < m := Nat.pos_of_neZero m
          have htop_val : (env.get (i₀ + (m - 1))).val
              = dtot / 2 ^ (B * (m - 1)) % 2 ^ B := by
            have h := hDn_eq (m - 1) (by omega : m - 1 < m)
            simp only [hDn, dif_pos (by omega : m - 1 < m)] at h
            simpa [hmpos] using h
          rw [show (Vector.map (Expression.eval env.toEnvironment)
              (Vector.mapRange m fun i => var { index := i₀ + i }))[m - 1]
              = env.get (i₀ + (m - 1)) by
            simp [circuit_norm]]
          rw [htop_val]
          ...
          rw [← pow_add]
          convert hdtot_tight using 2
          ring
      · -- 2. carry booleans (all but the top carry, whose zero value is folded
        -- into the top limb's constraint)
        intro i
        have hbit := hCn_bit i.val i.isLt
        rw [show env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val) + -1
          = env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val) - 1 by ring]
        exact (IsBool.iff_mul_sub_one).mp hbit
      · -- 3. per-limb field equation
        set Cn : ℕ → ℕ := fun k => if k = m - 1 then 0
          else (env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + k)).val with hCn
        -- the semantic top running carry is zero (`vb < 2^(B·m)`), so the folded
        -- `0` agrees with it
        have hPtop0 : P (m - 1) / 2 ^ (B * (m - 1 + 1)) = 0 := by
          have : P (m - 1) / 2 ^ (B * (m - 1 + 1)) = vb / 2 ^ (B * m) := by
            rw [hPlast, show m - 1 + 1 = m from by omega]
          rw [this, Nat.div_eq_of_lt hvb_lt]
        have hCnP : ∀ k, k < m → Cn k = P k / 2 ^ (B * (k + 1)) := by
          intro k hk
          by_cases hkt : k = m - 1
          · subst hkt
            simp only [hCn, if_pos rfl]
            exact hPtop0.symm
          · simp only [hCn, if_neg hkt]
            exact hCn_eq k (by omega)
        have hCtop0 : Cn (m - 1) = 0 := by simp [hCn]
        ...
        have hcin_e : Expression.eval env.toEnvironment
            (if h : i.val = 0 then 0 else var { index := i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (i.val - 1) })
            = if i.val = 0 then 0 else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (i.val - 1)) := by
          split <;> simp [circuit_norm]
        ...
        have hcout_e : Expression.eval env.toEnvironment
            (if h : i.val = m - 1 then 0 else var { index := i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val })
            = if i.val = m - 1 then (0 : F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val) := by
          split <;> simp [circuit_norm]
        ...
        have hCk : ((Cn i.val : ℕ) : F p)
            = (if i.val = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val)) := by
          split
          · rename_i htop; rw [htop, hCtop0]; simp
          · rename_i htop
            simp only [hCn, if_neg htop]; rw [ZMod.natCast_zmod_val]
        -- turn the goal into a cast of the ℕ equation
        have hcast_eq : (input.lhs[i.val]'hk) + env.get (i₀ + i.val)
            + (if i.val = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (i.val - 1)))
            + (if i.val = 0 then (1 : F p) else 0)
            = (input.rhs[i.val]'hk)
              + (if i.val = m - 1 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + i.val))
                * (2 ^ B : F p) := by
          have hcast := congrArg (Nat.cast : ℕ → F p) hnatk
          push_cast at hcast
          rw [hAk, hDk, hBk, hCk] at hcast
          -- rewrite the conditional casts
          rw [show ((if i.val = 0 then (0:F p) else env.get (i₀ + m + ((m - 1) * (B - 1) + (tb - 1)) + (i.val - 1))))
                = ((if i.val = 0 then (0:ℕ) else Cn (i.val - 1) : ℕ) : F p) by
              split
              · simp
              · rename_i h0
                simp only [hCn, if_neg (show ¬ (i.val - 1 = m - 1) by omega)]

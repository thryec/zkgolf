...
divided by `2^(B·(k+1))` is `≤ (m+1)·(2^B+2)` (the carry-offset bound). Uses the
exact geometric identity `(Σ_{j≤k} 2^(Bj))·(2^B−1) = 2^(B(k+1))−1` rather than
the lossy `sum ≤ 2·last` estimate, which is what allows the tighter offset. -/
lemma partial_div_bound (B m : ℕ) (hB1 : 1 ≤ B) (f : ℕ → ℕ)
  ...
  -- goal: (m+1)·2^(2B)·Σ ≤ 2^(B·(k+1)) · ((m+1)·(2^B+2)); compare after
  -- multiplying both sides by (2^B − 1) > 0 and using the geometric identity
...
satisfies `|c_k| ≤ (m+1)·2^(2B)/(2^B−1) ≤ (m+1)·(2^B+2)` when the coefficients
are `< (m+1)·2^(2B)` (exact geometric sum, not the lossy `sum ≤ 2·last` bound),
so `OFF = (m+1)·(2^B+2)` keeps `OFF + c_k ∈ [0, 2·OFF] ⊆ [0, 2^W)`. The tighter
offset lets the carry width drop to `W = 128` for the 4096-bit instance. -/
def carryOffset (B : ℕ) : ℕ := (m + 1) * (2 ^ B + 2)

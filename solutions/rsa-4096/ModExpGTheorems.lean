...
/-!
# A shallow closed form for the `ModExpG` output variable
...
lemma modExpLoop_output (P : BigIntParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p)
    (htbB : tb + 1 ≤ P.B) (g : ℕ) (hgp : GroupedEq.GHyps p m P.B P.W g)
    ...
    ∀ (rest : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ),
      (modExpLoop P tb htb htbB g hgp V hgv hNf base n (bit :: rest) acc).output offset
        = varFromOffset (BigInt m)
            (offset + prefixLen (squareModLen (m := m) P.B P.W tb g V.Wf) (mulModLen (m := m) P.B P.W tb g) (bit :: rest)
              + m) := by
  have hSL : ∀ (x : Var (SquareModLazy.Inputs m) (F p)),
      (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).localLength x = squareModLen (m := m) P.B P.W tb g V.Wf := fun _ => rfl
  have hML : ∀ (x : Var (MulMod.Inputs m) (F p)),
      (MulModLazyG.circuit P tb htb (Nat.le_of_succ_le htbB) g hgp).localLength x = mulModLen (m := m) P.B P.W tb g := fun _ => rfl
  have hstep : ∀ (bs : List Bool) (acc' : Var (BigInt m) (F p)) (o : ℕ),
      (modExpLoop P tb htb htbB g hgp V hgv hNf base n bs acc' o).1 = (modExpLoop P tb htb htbB g hgp V hgv hNf base n bs acc').output o := fun _ _ _ => rfl
  suffices H : ∀ (bs : List Bool) (acc : Var (BigInt m) (F p)) (offset : ℕ), bs ≠ [] →
      (modExpLoop P tb htb htbB g hgp V hgv hNf base n bs acc).output offset
        = varFromOffset (BigInt m)
            (offset + prefixLen (squareModLen (m := m) P.B P.W tb g V.Wf) (mulModLen (m := m) P.B P.W tb g) bs + m) by
  ...
  | cons b rest ih =>
    intro acc offset _
    set S := squareModLen (m := m) P.B P.W tb g V.Wf with hS
    set L := mulModLen (m := m) P.B P.W tb g with hL
    cases b
    · -- clear bit: 1 square, then loop on `rest`
      show (modExpLoop P tb htb htbB g hgp V hgv hNf base n rest
          (Vector.mapRange m fun i => var { index := offset + m + i })
          (offset + (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).localLength { a := acc, modulus := n })).1 = _
      rw [hSL, hstep]
      cases rest with
      | nil =>
        rw [show (modExpLoop P tb htb htbB g hgp V hgv hNf base n [] (Vector.mapRange m fun i => var { index := offset + m + i })).output
    ...
    · -- set bit: square + multiply, then loop on `rest`
      show (modExpLoop P tb htb htbB g hgp V hgv hNf base n rest
          ((MulModLazyG.circuit P tb htb (Nat.le_of_succ_le htbB) g hgp).output
            { a := (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).output { a := acc, modulus := n } offset,
              b := base, modulus := n }
            (offset + (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).localLength { a := acc, modulus := n }))
          (offset + (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).localLength { a := acc, modulus := n } +
            (MulModLazyG.circuit P tb htb (Nat.le_of_succ_le htbB) g hgp).localLength
              { a := (SquareModLazyG.circuit P tb htb htbB g V hgv hNf).output { a := acc, modulus := n } offset,
...
lemma main_output_of_tail (P : RSAParams p m) (tb : ℕ) (htb : 1 ≤ tb ∧ 2 ^ tb < p)
    ...
    (hNf : SquareModLazyG.NfOk (m := m) P.bigIntParams.B tb V) [Fact (p > 2)]
    (input : Var (Inputs m) (F p)) (offset : ℕ)
    (headBit : Bool) (b2 : Bool) (r2 : List Bool)
    (h : eBits P.e = headBit :: b2 :: r2) :
    (main P tb htb htbB g hgp V hgv hNf input).output offset
      = varFromOffset (BigInt m)
          (offset + prefixLen (squareModLen (m := m) P.bigIntParams.B P.bigIntParams.W tb g V.Wf)
              (mulModLen (m := m) P.bigIntParams.B P.bigIntParams.W tb g) (b2 :: r2)
            + m) := by
  simp only [main, h]
  show (modExpLoop P.bigIntParams tb htb htbB g hgp V hgv hNf input.base input.modulus (b2 :: r2) input.base).output offset = _
  exact modExpLoop_output P.bigIntParams tb htb htbB g hgp V hgv hNf input.base input.modulus b2 r2 input.base offset

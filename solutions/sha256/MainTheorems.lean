...
lemma paddedBytes_idx_lt {b w k : ℕ} (hb : b < 4) (hw : w < 16) (hk : k < 4) :
    b * 64 + 4 * w + k < paddedBytesLen := by
  simp only [paddedBytesLen, witnessedBlocksLen]; omega
...
lemma paddedBlock_word_eq (padded : Var SHA256PaddedBits (F circomPrime))
    (b : Fin witnessedBlocksLen) (w : Fin 16) {k : ℕ} (hk : k < 4) :
...
lemma wordByteVal_block (env : Environment (F circomPrime))
    ...
    (b : Fin witnessedBlocksLen) (w : Fin 16) (k : Fin 4) :
...
lemma paddedBlock_normalized (env : Environment (F circomPrime))
    ...
    (b : Fin witnessedBlocksLen) (w : ℕ) (hw : w < 16) :
  ...
  have hidx : b.val * 16 * 32 + w * 32 + bit.val < paddedBitsLen := by
    ...
    have := bit.isLt
    simp only [paddedBitsLen, witnessedBlocksLen]; omega

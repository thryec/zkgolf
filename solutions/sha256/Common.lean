...
The prover witnesses the first `witnessedBlocksLen * 16 * 32` padded bits,
laid out as `witnessedBlocksLen` blocks of 16 words of 32 bits. Within a word,
...
Byte index `j ∈ [0, witnessedBlocksLen*64)` of the witnessed padded message lives in
...
/-- Number of SHA-256 blocks whose bits are prover-witnessed.

Block 5 is computed from length flags by `CompressBlock5`, so its padded bits
are not witnessed or byte-checked here. -/
@[reducible] def witnessedBlocksLen : ℕ := 4

/-- A prover-witnessed padded SHA-256 message, flattened as `4 * 16 * 32` block bits. -/
@[reducible] def paddedBitsLen : ℕ := witnessedBlocksLen * 16 * 32

/-- Number of bytes in the witnessed padded-message buffer. -/
@[reducible] def paddedBytesLen : ℕ := witnessedBlocksLen * 64
...
lemma paddedBitIndex_lt {j t : ℕ} (hj : j < paddedBytesLen) (ht : t < 8) :
  ...
  simp only [paddedBytesLen, paddedBitsLen, witnessedBlocksLen] at hj ⊢
  omega
...
def paddedBit
    ...
    (block : Fin witnessedBlocksLen) (word : Fin 16) (bit : Fin 32) :
    Expression (F p) :=
  padded[block.val * 16 * 32 + word.val * 32 + bit.val]'(by
    have hb : block.val < 4 := block.isLt
    have hw := word.isLt
    have hbit := bit.isLt
    change block.val * 16 * 32 + word.val * 32 + bit.val < 4 * 16 * 32
    omega)

/-- Block `block` of the flat padded-bit vector, as a 16-word variable block. -/
def paddedBlock
    (padded : Var SHA256PaddedBits (F p))
    (block : Fin witnessedBlocksLen) : Var SHA256Block (F p) :=
...
def paddedWord
  ...
  Vector.ofFn fun (bit : Fin 32) =>
    padded[j.val / 64 * 512 + j.val % 64 / 4 * 32 + bit.val]'(by
      have hj := j.isLt
      have hb := bit.isLt
      simp only [paddedBytesLen, witnessedBlocksLen] at hj
      change _ < 4 * 16 * 32
      omega)
